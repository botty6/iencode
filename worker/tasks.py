...
async def _run_async_task(task_id: str, user_id: int, status_message_id: int, list_of_message_ids: list, quality: str, original_thumbnail_id: str, user_settings: dict):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if final_thumbnail_id:
                thumb_path = await app.download_media(final_thumbnail_id, file_name=os.path.join(temp_dir, "thumb.jpg"))
            ...
            await app.send_message(user_id, f"🚀 Job for `{output_filename}` finished!")
...
def encode_video_task(self, user_id: int, status_message_id: int, list_of_message_ids: list, quality: str, original_thumbnail_id: str, user_settings: dict):
    try:
        task_id = self.request.id
        # Here we could update the job status in MongoDB to "Processing"
        asyncio.run(_run_async_task(task_id, user_id, status_message_id, list_of_message_ids, quality, original_thumbnail_id, user_settings))
        # Here we could update the job status to "Completed" or remove it
    except Exception as e:
        logging.warning(f"Task {self.request.id} failed. Attempt {self.request.retries + 1}. Retrying... Error: {e}")
        # Here we could update the job status to "Retrying"
